...
/* activate redirection */    
   ...
   var element=SFM(this);
        ...
        {
            case "main_rss" :       SFM(element).removeClass('sfrd_finish');    
                    ...
                    if(res.response=='success')
                    {
                      ...
                      if (res.res_data.feed_id) {
                        sfmProcessFeeds(res.res_data.feed_id);
                         ...
                         switch(redirection_type)
                            {
                               case "main_rss"      :   SFM(element).removeClass('sfrd_process');
                               ...
                               case "comment_rss"   :     
                               case "category_rss"  :     
                               case "author_rss"    :   ProcessAfterActive(element,res.res_data);  
                               break;
                               case "custom_rss"    :   ProcessAfterCustomActive(res.res_data)
       ...
       SFM.ajax({
                ...
                {                   
                    if(res.response=='success')
                    {
                         ...
                         switch(redirection_type)
                            {
                                case "main_rss"     :   SFM('#main_rss').removeClass('sfrd_finish');
                               ...
                               case "comment_rss"   :     
                               case "category_rss"  :     
                               case "author_rss"    :   ProcessAfterReverse(element,res);  
                               break;
                               case "custom_rss"    :   SFM(element).parent().parent().slideUp('slow',function(){ SFM(element).parent().parent().remove(); updateCustomFeedNos();});
...
var content='<div class="sfrd_feedmaster_tab sfm_customFeeds" style="display:none">';
    ...
    content+='<span> <a id="custom_'+res.feed_id+'" target="_new" title="'+res.feed_url+'" href="'+res.feed_url+'">'+res.feed_url+'</a></span>'
    content+='</h3>';
    content+="<small>";
    ...
    SFM('custom_'+res.feed_id).parent().parent().parent().next('.sfrd_green_box1').find('#mainRssconnect').attr('href',res.connect_string);
